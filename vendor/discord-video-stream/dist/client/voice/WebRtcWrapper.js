import { PeerConnection, Audio, Video, PacingHandler, RtpPacketizer, H264RtpPacketizer, H265RtpPacketizer, AV1RtpPacketizer, RtpPacketizationConfig, RtcpNackResponder, RtcpSrReporter, } from "@lng2004/node-datachannel";
...
export class WebRtcConnWrapper {
    ...
    sendVideoFrame(frame, frametime) {
        if (!this.ready)
            return;
        if (!this._videoPacketizer)
            return;
        ...
        if (this.mediaConnection.daveReady) {
            let daveCodec = 0 /* Codec.UNKNOWN */;
            switch (this._videoCodec) {
                case "H264":
                    daveCodec = 4 /* Codec.H264 */;
                    break;
                case "H265":
                    daveCodec = 5 /* Codec.H265 */;
                    break;
                case "VP8":
                    daveCodec = 2 /* Codec.VP8 */;
                    break;
                case "VP9":
                    daveCodec = 3 /* Codec.VP9 */;
                    break;
                case "AV1":
                    daveCodec = 6 /* Codec.AV1 */;
                    break;
            }
            frame = this.mediaConnection.daveSession.encrypt(1 /* MediaType.VIDEO */, daveCodec, frame);
        }
    ...
    setPacketizer(videoCodec) {
        ...
        const rtpConfigVideo = new RtpPacketizationConfig(videoSsrc, "", CodecPayloadType[this._videoCodec].payload_type, CodecPayloadType[this._videoCodec].clockRate);
...
//# sourceMappingURL=WebRtcWrapper.js.map